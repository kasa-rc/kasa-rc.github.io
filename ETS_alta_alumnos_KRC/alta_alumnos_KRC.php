...
function ActionCreatePHP($conexion){
  ...
  $apellidoM 	= $_POST['apellidoM'];
	...
	$Query = "INSERT INTO alta_alumnos (id, programa, nombre, apellidoP, apellidoM) VALUES (NULL, '".$programa."',  '".$nombre."', '".$apellidoP."', '".$apellidoM."')";
...
function ActionUpdatePHP($conexion){
  ...
  $apellidoM 	= $_POST['apellidoM'];
    ...
    $Query ="UPDATE alta_alumnos SET  nombre='".$nombre."', apellidoP='".$apellidoP."', apellidoM='".$apellidoM."', programa='".$programa."' WHERE id=".$Id;
...
function ActionReadPHP($conexion){
	...
	$Query 		="SELECT * FROM alta_alumnos";

	$Respuesta["altas"]	=	array();
	...
	while($Renglon = mysqli_fetch_array($Resultado)){
		...
		array_push($Respuesta["altas"], $Tipo_Evento);
	}
	$Respuesta["estado"]	=1;
...
function ActionDeletePHP($conexion){
	...
	if(isset($_POST['id'])){
		...
		$Query = "DELETE FROM alta_alumnos WHERE id=".$Id;
